...
const Plate = ({ ordersList, getOrder }) => {
  ...
  const retiveList = async () => {
    ...
    setPlateDetails(responseData.Progress);
    setUpdate(false);
  };
  useEffect(() => {
    retiveList();
  ...
  const searchHandler = () => {
    const filter = plateDetails.filter(
      (f) =>
        f.name.toLowerCase() === search.toLowerCase() ||
        f.plateProgress.toLowerCase() === search.toLowerCase() ||
        f.plateCDate === search
    );
    setPlateDetails(filter);
    setSearch('');
  ...
  const sortHandler = () => {
    let orderSorted = plateDetails.sort(
      (a, b) =>
        new Date(...a.date.split('-').reverse()) -
        new Date(...b.date.split('-').reverse())
    );
    console.log(orderSorted);
    setPlateDetails(orderSorted);
  };
  ...
  const handleSelect = async (date) => {
    ...
    setPlateDetails(
      responseData.Progress.filter((product) => {
        let productDate = product['plateCDate'];
                ...
                (item) =>
                  item.plate === 'yes' && (
                    <tr key={item.orderNo}>
                      ...
                      <td>{item.plateCDate}</td>
                      <td>
                        <PlateProgress
                          item={item}
                          ordersList={plateDetails}
                          getOrder={getOrder}
                          setUpdate={setUpdate}
                        />
                      </td>
