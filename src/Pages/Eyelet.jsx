...
import './table.css';
import EyeletProgress from '../components/EyeletProgress';
...
const Eyelet = ({ ordersList, getOrder }) => {
  const [eyeletDetails, setEyeletDetails] = useState([]);
  ...
  const retiveList = async () => {
    ...
    setEyeletDetails(responseData.Progress);
    setUpdate(false);
  };
  useEffect(() => {
    retiveList();
  ...
  const searchHandler = () => {
    const filter = eyeletDetails.filter(
      (f) =>
        f.name.toLowerCase() === search.toLowerCase() ||
        f.eyeletProgress.toLowerCase() === search.toLowerCase() ||
        f.eyeletCDate === search
    );
    setEyeletDetails(filter);
    setSearch('');
  ...
  const sortHandler = () => {
    let orderSorted = eyeletDetails.sort(
      (a, b) =>
        new Date(...a.date.split('-').reverse()) -
        new Date(...b.date.split('-').reverse())
    );
    console.log(orderSorted);
    setEyeletDetails(orderSorted);
  };
  ...
  const handleSelect = async (date) => {
    ...
    setEyeletDetails(
      responseData.Progress.filter((product) => {
        let productDate = product['eyeletCDate'];
                ...
                (item) =>
                  item.eyelet === 'yes' && (
                    <tr key={item.orderNo}>
                      ...
                      <td>{item.eyeletCDate}</td>
                      <td>
                        <EyeletProgress
                          item={item}
                          ordersList={eyeletDetails}
                          getOrder={getOrder}
                          setUpdate={setUpdate}
                        />
                      </td>
