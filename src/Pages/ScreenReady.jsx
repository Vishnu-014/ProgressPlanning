...
const ScreenReady = ({ ordersList, getOrder }) => {
  ...
  const searchHandler = () => {
    const filter = screenReadyDetails.filter(
      (f) =>
        f.name.toLowerCase() === search.toLowerCase() ||
        f.screenPreparationProgress.toLowerCase() === search.toLowerCase() ||
        f.screenPreparationCDate === search
    );
    setScreenReadyDetails(filter);
    setSearch('');
  ...
  const sortHandler = () => {
    let orderSorted = screenReadyDetails.sort(
      (a, b) =>
        new Date(...a.date.split('-').reverse()) -
        new Date(...b.date.split('-').reverse())
    );
    console.log(orderSorted);
    setScreenReadyDetails(orderSorted);
  };
  ...
  const handleSelect = async (date) => {
    ...
    setScreenReadyDetails(
      responseData.Progress.filter((product) => {
        let productDate = product['screenPreparationCDate'];
