...
import './table.css';
import DieCuttingProgress from '../components/DieCuttingProgress';
...
const DieCutting = ({ ordersList, getOrder }) => {
  ...
  const retiveList = async () => {
    ...
    setDieCuttingDetails(responseData.Progress);
    setUpdate(false);
  };
  useEffect(() => {
    retiveList();
  ...
  const searchHandler = () => {
    const filter = dieCuttingDetails.filter(
      (f) =>
        f.name.toLowerCase() === search.toLowerCase() ||
        f.dieCuttingProgress.toLowerCase() === search.toLowerCase() ||
        f.dieCuttingCDate === search
    );
    setDieCuttingDetails(filter);
    setSearch('');
  ...
  const sortHandler = () => {
    let orderSorted = dieCuttingDetails.sort(
      (a, b) =>
        new Date(...a.date.split('-').reverse()) -
        new Date(...b.date.split('-').reverse())
    );
    console.log(orderSorted);
    setDieCuttingDetails(orderSorted);
  };
  ...
  const handleSelect = async (date) => {
    ...
    setDieCuttingDetails(
      responseData.Progress.filter((product) => {
        let productDate = product['dieCuttingCDate'];
