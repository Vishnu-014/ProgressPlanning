...
const Lamination = ({ ordersList, getOrder }) => {
  ...
  const retiveList = async () => {
    ...
    setLaminationDetails(responseData.Progress);
    setUpdate(false);
  };
  useEffect(() => {
    retiveList();
  ...
  const searchHandler = () => {
    const filter = laminationDetails.filter(
      (f) =>
        f.name.toLowerCase() === search.toLowerCase() ||
        f.laminationProgress.toLowerCase() === search.toLowerCase() ||
        f.laminationCDate === search
    );
    setLaminationDetails(filter);
    setSearch('');
  ...
  const sortHandler = () => {
    let orderSorted = laminationDetails.sort(
      (a, b) =>
        new Date(...a.date.split('-').reverse()) -
        new Date(...b.date.split('-').reverse())
    );
    console.log(orderSorted);
    setLaminationDetails(orderSorted);
  };
  ...
  const handleSelect = async (date) => {
    ...
    setLaminationDetails(
      responseData.Progress.filter((product) => {
        let productDate = product['laminationCDate'];
  ...
  return (
    <Fragment>
      <Container>
        <GlobalStyle />
        <Title>Lamination Progress</Title>
        <Search
          placeholder="Search"
                ...
                (item) =>
                  item.lamination === 'yes' && (
                    <tr key={item.orderNo}>
                      ...
                      <td>{item.laminationCDate}</td>
                      <td>
                        <LaminationProgress
                          item={item}
                          ordersList={laminationDetails}
                          getOrder={getOrder}
                          setUpdate={setUpdate}
                        />
                      </td>
