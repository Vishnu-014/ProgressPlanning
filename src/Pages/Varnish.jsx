...
import './table.css';
import VarnishProgress from '../components/VarnishProgress';
...
const Varnish = ({ ordersList, getOrder }) => {
  const [varnishDetails, setVarnishDetails] = useState([]);
  ...
  const retiveList = async () => {
    ...
    setVarnishDetails(responseData.Progress);
    setUpdate(false);
  };
  useEffect(() => {
    retiveList();
  ...
  const searchHandler = () => {
    const filter = varnishDetails.filter(
      (f) =>
        f.name.toLowerCase() === search.toLowerCase() ||
        f.varnishProgress.toLowerCase() === search.toLowerCase() ||
        f.varnishCDate === search
    );
    setVarnishDetails(filter);
    setSearch('');
  ...
  const sortHandler = () => {
    let orderSorted = varnishDetails.sort(
      (a, b) =>
        new Date(...a.date.split('-').reverse()) -
        new Date(...b.date.split('-').reverse())
    );
    console.log(orderSorted);
    setVarnishDetails(orderSorted);
  };
  ...
  const handleSelect = async (date) => {
    ...
    setVarnishDetails(
      responseData.Progress.filter((product) => {
        let productDate = product['varnishCDate'];
                ...
                (item) =>
                  item.varnish === 'yes' && (
                    <tr key={item.orderNo}>
                      ...
                      <td>{item.varnishCDate}</td>
                      <td>
                        <VarnishProgress
                          item={item}
                          ordersList={varnishDetails}
                          getOrder={getOrder}
                          setUpdate={setUpdate}
                        />
                      </td>
