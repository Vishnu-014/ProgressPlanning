...
const Design = ({ ordersList, getOrder }) => {
  ...
  const searchHandler = () => {
    const filter = designDetails.filter(
      (f) =>
        f.name.toLowerCase() === search.toLowerCase() ||
        f.designProgress.toLowerCase() === search.toLowerCase() ||
        f.designCDate === search
    );
    setDesignDetails(filter);
    setSearch('');
  ...
  const sortHandler = () => {
    let orderSorted = designDetails.sort(
      (a, b) =>
        new Date(...a.date.split('-').reverse()) -
        new Date(...b.date.split('-').reverse())
    );
    console.log(orderSorted);
    setDesignDetails(orderSorted);
  };
  ...
  const handleSelect = async (date) => {
    ...
    setDesignDetails(
      responseData.Progress.filter((product) => {
        let productDate = product['designCDate'];
